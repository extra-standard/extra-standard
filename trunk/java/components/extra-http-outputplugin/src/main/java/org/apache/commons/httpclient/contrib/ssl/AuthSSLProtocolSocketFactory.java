...
public class AuthSSLProtocolSocketFactory implements
	...
	private static KeyStore createKeyStore(final URL url, final String password)
		...
		logger.trace("Initializing key store");
	...
	private static KeyManager[] createKeyManagers(final KeyStore keystore,
		...
		logger.trace("Initializing key manager");
	...
	private static TrustManager[] createTrustManagers(final KeyStore keystore)
		...
		logger.trace("Initializing trust manager");
	...
	private SSLContext createSSLContext() {
		try {
			...
			if (this.keystoreUrl != null) {
				...
				if (logger.isTraceEnabled()) {
					...
					while (aliases.hasMoreElements()) {
						...
						if (certs != null) {
							logger.trace("Certificate chain '" + alias + "':");
							for (int c = 0; c < certs.length; c++) {
								if (certs[c] instanceof X509Certificate) {
									X509Certificate cert = (X509Certificate) certs[c];
									logger.trace(" Certificate " + (c + 1)
											+ ":");
									logger.trace(" Subject DN: "
											+ cert.getSubjectDN());
									logger.trace(" Signature Algorithm: "
											+ cert.getSigAlgName());
									logger.trace(" Valid from: "
											+ cert.getNotBefore());
									logger.trace(" Valid until: "
											+ cert.getNotAfter());
									logger.trace(" Issuer: "
											+ cert.getIssuerDN());
								}
			...
			// sslcontext.init(new KeyManager[0], trustmanagers, new
			// SecureRandom());

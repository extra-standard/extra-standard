...
public class PdfCreatorImpl implements PdfCreator {
	...
	private String fontNameStandard = FontFactory.HELVETICA;

	private int fontSizeChapter = 18;
	private int fontSizeElement = 14;
	private int fontSizeText = 11;
	private int fontSizeFooter = 9;
	private int fontSizeTextHalf = 5;

	// Aufzählungszeichen für die erste und zweite Ebene
	private String bulletMain = "\u2022";
	private String bulletSub = "-";
	...
	/**
	 * Erzeugt die PDF-Doku für das übergebene SchemaSet und optional mit Inhaltsverzeichnis
	 * 
	 * @param xmlSchemaSet
	 ...
	 * @throws IOException
	 */
	private void erzeugePdfDatei(XSSchemaSet xmlSchemaSet, boolean writeInhaltsverzeichnis) throws DocumentException,
		...
		String strRootElement = configurator.getPropertySystem(Configurator.PropBezeichnungSystem.SCHEMA_ROOT_ELEMENT);
		for (XSSchema currSchema : xmlSchemaSet.getSchemas()) {
			// Neue Queue für aktuelles Schema anlegen
			String nsPrefix = getNsPref(currSchema.getTargetNamespace());
			schemaQueues.put(nsPrefix, new LinkedList<XSElementDecl>());

			// Prüfen, ob erstes Element in diesem Schema enthalten ist
			XSElementDecl currFirstElement = currSchema.getElementDecl(strRootElement);
			if (currFirstElement != null) {
				// Element als erstes Element dieser Queue hinzufügen
	...
	/**
	 * Erzeugt das Inhaltsverzeichnis aus den bereits vorhandenen Elementen in der Liste <code>listEntries</code>.
	 * 
	 * @param docPdf
	 *            Zieldokument, falls Inhaltsverzeichnis nicht temporär erzeugt wird
	 * @param temp
	 *            Gibt an, ob das Inhaltsverzeichnis temporär in einer neuen Datei/Dokument erzeugt werden soll
	...
	private int erzeugeInhaltsverzeichnis(Document docPdf, boolean temp) throws DocumentException, IOException {
		...
		for (ContPdfEntry currEntry : listEntries) {
			...
			if (currEntry.getParentEntry() == null) {
				// 1. Ebene => fett, Abstand davor einfügen
				docInhalt.add(getEmptyLineTextHalf());
			...
			// Abstandzeichen generieren, Breite auffüllen
	...
	/**
	 * Alle Elemente, die sich für das angegebene Schema in der entsprechenden Queue befinden, werden behandelt.
	 * 
	 * @param currSchemaPrefix
	 *            Präfix des zu behandelnden Schemas
	 * @param schemaQueues
	 *            Map enthält Queues für jedes Schema (Key ist der Namepace-Präfix)
	 * @param xmlSchemaSet
	...
	private void behandleSchemaQueue(String currSchemaPrefix, Map<String, Queue<XSElementDecl>> schemaQueues,
		...
		if (currQueue != null) {
			...
			// zur Sicherheit nochmal alle Elemente dieses Schema in Queue einfügen
			// damit ist sichergestellt, dass auch nicht direkt verknüpfte Elemente (any => Plugins) behandelt werden
			...
			if (currQueue.peek() != null) {
				...
				// Kapitel dem Dokument hinzufügen
				docPdf.add(currChapter);

				// Eintrag für Inhaltsverzeichnis (Chapter)
	 ...
	 * @param currChapter
	 *            Kapitel, zu dem das Element hinzugefügt wird
	 * @param chapterEntry
	 *            Eintrag des Kapitels für das Inhaltsverzeichnis
	 * @param schemaQueues
	 *            Queues mit allen Elementen für die einzelnen Schemas
	 */
	private void behandleElement(XSElementDecl currElement, Chapter currChapter, ContPdfEntry chapterEntry,
		...
		// Für jedes Element eine Section; Titel ist der Elementname
		...
		// Eintrag für Inhaltsverzeichnis (Element)
		listEntries.add(new ContPdfEntry(chapterEntry, currSection.getTitle().getContent(),
				currReferenzString, currPageNumber));

		// Sektion dem Dokument (vorläufig) hinzufügen (wichtig für das Aktualisieren der Seitenzahl)
		...
		// Sektion dem Dokument (endgültig) hinzufügen
		currSection.setComplete(true);
		docPdf.add(currSection);
	}

	/**
	 * Die Kindelemente (Sequence oder Choice) des übergebenen Elements werden behandelt
	 * 
	...
	private void behandleKindElemente(XSElementDecl currElementMain, Section currSection,
		...
		if ((childs != null) && (childs.length > 0)) {
			// Für jedes Kind wird ein Listeneintrag erstellt
	...
	/**
	 * Für die angegebenen Kindlemente wird ein Listeneintrag für das PDF erzeugt.
	 * 
	 * @param childs
	 *            Kindelemente
	 * @param schemaQueues
	 *            Queues mit allen Elementen für die einzelnen Schemas
	 * @param strBullet
	 *            Zeichen, das als Aufzählunsgzeichen für die Liste verwendet wird
	...
	/**
	 * Für die Attribute des aktuellen Typs wird eine Liste erzeugt und der angegebenen Sektion hinzugefügt.
	 * 
	 * @param currType
	 *            Schemtyp, für den die Attribute bestimmt werden
	 * @param currSection
	...
	private void behandleAttribute(XSType currType, Section currSection) {
		if (currType.isComplexType()) {
			...
			@SuppressWarnings("unchecked")
			Iterator<XSAttributeUse> itr2 = (Iterator<XSAttributeUse>) currType.asComplexType().iterateAttributeUses();
	...
	/**
	 * Diese Methode erzeugt die Titelseite für das PDF-Dokument mit der Bezeichnung des Verfahrens.
	 * 
	 * @param bezVerfahren
	...
	private void erzeugeTitelSeite() throws DocumentException, MalformedURLException, IOException {
		...
		try {
			// Image image = Image.getInstance(PdfCreatorImpl.class.getResource(pathImage));
	...
	private String getMinMaxStr(XSParticle currParticle) {
		// minOccurs
		int minOccursChild = currParticle.getMinOccurs();
		String strminOccurs = "minOccurs: " + minOccursChild;
		// maxOccurs
		int maxOccursChild = currParticle.getMaxOccurs();
	...
	private Chunk getChunkTextBoldWithReference(String text, String reference) {
		...
		// Unterstreichen
		chunkText.setUnderline(new BaseColor(0x00, 0x0f, 0xFF), 0.5f, 0.0f, -4f, 0.0f, PdfContentByte.LINE_CAP_BUTT);
		return chunkText;
	}
	...
	class HeaderFooter extends PdfPageEventHelper {
		...
		public void onEndPage(PdfWriter writer, Document document) {
			...
			if (writer.getPageNumber() == 1) {
				...
				// Titel des Verfahrens (ungefähr in der Mitte der Seite)
				...
				ColumnText
						.showTextAligned(contentByte, Element.ALIGN_CENTER, phraseTitle, posTitleX, posTitleY + 30, 0);
				...
				ColumnText.showTextAligned(contentByte, Element.ALIGN_RIGHT, phraseDatum, posRight, document
						.bottomMargin(), 0);

			} else if (writer.getPageNumber() > 1) {
				// Kopf- und Fußzeile erst ab der zweiten Seite ...
				...
				ColumnText.showTextAligned(contentByte, Element.ALIGN_LEFT, phraseDateiname, posLeft, posTopText, 0);
				...
				ColumnText.showTextAligned(contentByte, Element.ALIGN_RIGHT, phraseFooter, posRight, posBottomText, 0);
			}

...
public class PropertyPlaceholderPluginConfigurer extends
	...
	private void setPluginsProperties(
			final ConfigurableListableBeanFactory beanFactory,
			final Properties properties, final String beanName,
			final Class<?> clazz) {
		final PluginConfiguration annotationConfigutation = clazz
				.getAnnotation(PluginConfiguration.class);
		final MutablePropertyValues mutablePropertyValues = beanFactory
				.getBeanDefinition(beanName).getPropertyValues();

		for (final PropertyDescriptor property : BeanUtils
			...
			if (valueAnnotation != null) {
				...
				final String value = resolvePlaceholder(key, properties,
		...
		for (final Field field : clazz.getDeclaredFields()) {
			...
			if (field.isAnnotationPresent(PluginValue.class)) {
				final PluginValue valueAnnotation = field
						.getAnnotation(PluginValue.class);
				final PropertyDescriptor property = BeanUtils
	...
	private String extractKey(final PluginConfiguration annotation,
		...
		try {
			...
			final String configPrefix = annotation.pluginType()
					.getConfigPrefix();

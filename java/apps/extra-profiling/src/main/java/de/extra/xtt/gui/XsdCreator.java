...
public class XsdCreator extends JFrame implements TreeSelectionListener {
	...
	private XsdCreatorCtrl xsdCreatorCtrl;
	...
	private JPanel getJPanelLeft() {
		if (jPanelLeft == null) {
			...
			jLabelNewKonfig.setText(configurator.getResString("XSDCREATOR_LABEL_NEUE_PROFILKONFIGURATION"));
			...
			labelTextBtnSchema.setText(configurator.getResString("XSDCREATOR_LABEL_TEXTBTNSCHEMA"));
	...
	private JPanel getJPanelMainHeader() {
		if (jPanelMainHeader == null) {
			...
			jPanelMainHeader.setComponentOrientation(ComponentOrientation.LEFT_TO_RIGHT);
			...
			gbl_jPanelMainHeader.rowWeights = new double[]{0.25, 0.25, 0.25, 0.25};
			...
			jLabelSchemaLab.setText(configurator.getResString("XSDCREATOR_LABEL_SCHEMDATEI"));
			...
			jLabelKonfigLab.setText(configurator.getResString("XSDCREATOR_LABEL_KONFIGURATIONSDATEI"));
			...
			jPanelMainHeader.add(getLblBezVerfahren(), gbc_lblBezVerfahren);
						...
						lblBezKurzVerfahren.setText(configurator.getResString("XSDCREATOR_LABEL_BEZEICHNUNGKURZVERFAHREN"));
	...
	public void valueChanged(TreeSelectionEvent e) {
		currNode = (ProfilingTreeNode) ((JTree) e.getSource()).getLastSelectedPathComponent();
	...
	private void setValuesCurrentSelectedNode() {
		if (currNode == null) {
			...
			// Text zum selektierten Element
			jLabelSelectedElement.setText(currNode.getSchemaElement().getNameWithPrefix());
			...
			if (currNode.isMinOccursChangeable()) {
				// aktuellen Wert und Beschränkung setzen
				spinnerMinOccurs.setModel(new SpinnerNumberModel(currNode.getMinOccursUser(), currNode
						.getMinOccursDefault(), currNode.getMaxOccursDefault(), 1));
			} else {
				spinnerMinOccurs.setValue(0);
			}
			if (currNode.isMaxOccursChangeable()) {
				// aktuellen Wert setzen
				spinnerMaxOccurs.setModel(new SpinnerNumberModel(currNode.getMaxOccursUser(), currNode
						.getMinOccursDefault(), currNode.getMaxOccursDefault(), 1));
			} else {
			...
			// Anmerkungen, allgemein
			fAnmerkAllgemein.setText(configurator.getAnmerkungAllgemein(currNode));
			fAnmerkAllgemein.setCaretPosition(0);
			fAnmerkAllgemein.setEnabled(true);
			btnOkAnmerkungenAllg.setEnabled(true);

			// Anmerkungen, zur Verwendung
			if ((currNode.getParent() == null) || currNode.getParent().isRootReferencedElements(configurator)
					|| currNode.belongsToWildcard() || currNode.isLocalElement()) {
				// - kein Vaterknoten
				// - Vaterknoten ist Wurzel der mehrfach referenzierten Elemente
				// - aktuelles Element ist Teil einer any-Sequenz
				// - aktuelles Element wird im Schema lokal definiert (es reicht eine Anmerkung)
				// => keine Verwendungsanmerkung
				fAnmerkVerwendung.setText(configurator.getResString("XSDCREATOR_TEXTPANE_ANMERKUNGENVERWENDUNG"));
				fAnmerkVerwendung.setEnabled(false);
				btnOkAnmerkungenVerwendung.setEnabled(false);
			} else {
				fAnmerkVerwendung.setText(configurator.getAnmerkungVerwendung(currNode));
				fAnmerkAllgemein.setCaretPosition(0);
	...
	private JButton getJButtonNewForRequest() {
		if (jButtonNewForRequest == null) {
			...
			jButtonNewForRequest.setText(configurator.getResString("XSDCREATOR_BTN_RQUEST"));
			jButtonNewForRequest.setSize(new Dimension(100, 26));
			jButtonNewForRequest.setLocation(new Point(15, 35));
			jButtonNewForRequest.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					XsdCreator.this.setCursor(new Cursor(Cursor.WAIT_CURSOR));
					int n = JOptionPane.YES_OPTION;
					if ((treeMain != null) && (treeMain.getModel() != null)) {
						n = JOptionPane.showConfirmDialog(jContentPane, configurator
								.getResString("XSDCREATOR_DLG_ANPASSUNGEN_VERWERFEN"), configurator
								.getResString("XSDCREATOR_DLG_NEUE_PROFILIERUNG"), JOptionPane.YES_NO_OPTION);
					}
					if (n == JOptionPane.YES_OPTION) {
						try {
							setCurrFileKonfigToNull(xsdCreatorCtrl.createTreeModelForRequest());
						} catch (XsdCreatorCtrlException e1) {
							logger.error("Fehler beim Anzeigen des Request-Schemas:", e1);
							JOptionPane.showMessageDialog(null, configurator
									.getResString("XSDCREATOR_DLG_FEHLER_TREEMODEL_REQUEST"), configurator
									.getResString("XSDCREATOR_DLG_NEUE_PROFILIERUNG"), JOptionPane.ERROR_MESSAGE);
						}
					}
					XsdCreator.this.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
				}
			});
		}
	...
	private void setCurrFileKonfigAndRefreshView(File currFileKonfig) throws XsdCreatorCtrlException {
		xsdCreatorCtrl.setXmlFileConfig(currFileKonfig);
		if (currFileKonfig != null) {
			// Neuen Schema-Baum erzeugen und anzeigen
			ProfilingTreeModel treeModelMain = xsdCreatorCtrl.createTreeModelForCurrentConfig();
			...
			if (treeModelMain.getTreeModelRefElements() != null) {
				...
				scrollPaneAdditional.setViewportView(getTreeAdditional(treeModelMain.getTreeModelRefElements()));
			} else {
	...
	private void setCurrFileKonfigToNull(ProfilingTreeModel treeModel) throws XsdCreatorCtrlException {
		...
		if (treeModel.getTreeModelRefElements() != null) {
			// Baum mit referenzierten Elementen anzeigen
			scrollPaneAdditional.setViewportView(getTreeAdditional(treeModel.getTreeModelRefElements()));
		} else {
			scrollPaneAdditional.setViewportView(null);
		}
		refreshTextAndButtons();
	...
	private JButton getJButtonNewForRespone() {
		if (jButtonNewForRespone == null) {
			...
			jButtonNewForRespone.setText(configurator.getResString("XSDCREATOR_BTN_RESPONSE"));
			jButtonNewForRespone.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					XsdCreator.this.setCursor(new Cursor(Cursor.WAIT_CURSOR));
					int n = JOptionPane.YES_OPTION;
					if ((treeMain != null) && (treeMain.getModel() != null)) {
					...
					if (n == JOptionPane.YES_OPTION) {
						try {
							setCurrFileKonfigToNull(xsdCreatorCtrl.createTreeModelForResponse());
						} catch (XsdCreatorCtrlException e1) {
							logger.error("Fehler beim Anzeigen des Reponse-Schemas:", e1);
							JOptionPane.showMessageDialog(null, configurator
									.getResString("XSDCREATOR_DLG_FEHLER_TREEMODEL_RESPONSE"), configurator
									.getResString("XSDCREATOR_DLG_NEUE_PROFILIERUNG"), JOptionPane.ERROR_MESSAGE);
						}
					}
					XsdCreator.this.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
	...
	private JButton getJButtonSaveKonfig() {
		if (jButtonSaveKonfig == null) {
			...
			jButtonSaveKonfig.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					XsdCreator.this.setCursor(new Cursor(Cursor.WAIT_CURSOR));
					saveKonfig(false);
					XsdCreator.this.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
			...
			jButtonSaveKonfig.setText(configurator.getResString("XSDCREATOR_BTN_SPEICHERN_KONFIG"));
	...
	private JButton getJButtonLoadKonfig() {
		if (jButtonLoadKonfig == null) {
			...
			jButtonLoadKonfig.setText(configurator.getResString("XSDCREATOR_BTN_KONFIGURATION_LADEN"));
			jButtonLoadKonfig.setSize(new Dimension(220, 26));
			jButtonLoadKonfig.setLocation(new Point(15, 90));
			jButtonLoadKonfig.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					XsdCreator.this.setCursor(new Cursor(Cursor.WAIT_CURSOR));
					int n = JOptionPane.YES_OPTION;
					if ((treeMain != null) && (treeMain.getModel() != null)) {
						n = JOptionPane.showConfirmDialog(jContentPane, configurator
								.getResString("XSDCREATOR_DLG_ANPASSUNGEN_VERWERFEN"), configurator
								.getResString("XSDCREATOR_DLG_KONFIGURATION_LADEN"), JOptionPane.YES_NO_OPTION);
					}
					if (n == JOptionPane.YES_OPTION) {
						try {
							...
							logger.error("Fehler beim Laden der Profil-Konfiguration:", ex);
							JOptionPane.showMessageDialog(null, configurator
									.getResString("XSDCREATOR_DLG_FEHLER_LADEN_KONFIG"), configurator
									.getResString("XSDCREATOR_DLG_KONFIGURATION_LADEN"), JOptionPane.ERROR_MESSAGE);
						}
					}
					XsdCreator.this.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
				}
	...
	private JButton getJButtonCreateSchema() {
		if (jButtonCreateSchema == null) {
			...
			jButtonCreateSchema.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					XsdCreator.this.setCursor(new Cursor(Cursor.WAIT_CURSOR));
					...
					try {
						Map<String, Document> docXsd = xsdCreatorCtrl.createSchemaProf();
						JFileChooser fc = new JFileChooser();
						fc.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
						int returnVal = fc.showSaveDialog(null);
						if (returnVal == JFileChooser.APPROVE_OPTION) {
							selectedPath = fc.getSelectedFile().getPath();
							...
							for (Map.Entry<String, Document> currEntry : docXsd.entrySet()) {
								String currFilePath = selectedPath
										+ "\\"
										+ configurator.getDateinameFuerSchema(xsdCreatorCtrl
												.getBezeichnungKurzVerfahren(), currEntry.getKey());
								File file = new File(currFilePath);
								if (currEntry.getKey().equals("xreq") || currEntry.getKey().equals("xres")) {
							...
							if (dateiVorhanden) {
								...
								n = JOptionPane.showConfirmDialog(jContentPane, configurator
										.getResString("XSDCREATOR_DLG_DATEI_BEREITS_VORHANDEN_FRAGE"), configurator
										.getResString("XSDCREATOR_DLG_DATEI_BEREITS_VORHANDEN"),
										JOptionPane.YES_NO_OPTION);
							}
							if (n == JOptionPane.YES_OPTION) {
								// Schema-Datei speichern
								xsdCreatorCtrl.saveXsdSchema(selectedPath, docXsd);
								// Frage, ob Dokumentation derstellt werden soll
								dlgErzeugeDoku = JOptionPane.showConfirmDialog(jContentPane, configurator
										.getResString("XSDCREATOR_DLG_PDFDOKU_ERZEUGEN_FRAGE"), configurator
										.getResString("XSDCREATOR_DLG_PDFDOKU_ERZEUGEN"), JOptionPane.YES_NO_OPTION);
							}
						}
					} catch (XsdCreatorCtrlException e1) {
						logger.error("Fehler beim Erstellen/Speichern des profilierten Schemas:", e1);
						JOptionPane.showMessageDialog(null, configurator
								.getResString("XSDCREATOR_DLG_FEHLER_NEUES_SCHEMA"), configurator
								.getResString("XSDCREATOR_DLG_NEUES_SCHEMA"), JOptionPane.ERROR_MESSAGE);
					}

					if (dlgErzeugeDoku == JOptionPane.YES_OPTION) {
						// PDF-Dokumentation erzeugen
						String currFilePath = selectedPath + "\\"
								+ configurator.getDateinameFuerDoku(xsdCreatorCtrl.getBezeichnungKurzVerfahren());
						try {
							xsdCreatorCtrl.createPdfDoku(currFilePath, pathMainSchema);
						} catch (XsdCreatorCtrlException e1) {
							logger.error("Fehler beim Erstellen der PDF-Dokumentation für das profilierte Schema:", e1);
							JOptionPane.showMessageDialog(null, configurator
									.getResString("XSDCREATOR_DLG_FEHLER_PDFDOKU_ERZEUGEN"), configurator
									.getResString("XSDCREATOR_DLG_PDFDOKU_ERZEUGEN"), JOptionPane.ERROR_MESSAGE);
						}
					}
			...
			jButtonCreateSchema.setText(configurator.getResString("XSDCREATOR_BTN_NEUES_SCHEMA"));
	...
	private JButton getJButtonSaveAsKonfig() {
		if (jButtonSaveAsKonfig == null) {
			...
			jButtonSaveAsKonfig.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					XsdCreator.this.setCursor(new Cursor(Cursor.WAIT_CURSOR));
					saveKonfig(true);
					XsdCreator.this.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
			...
			jButtonSaveAsKonfig.setText(configurator.getResString("XSDCREATOR_BTN_SPEICHERN_UNTER_KONFIG"));
	...
	private static Configurator initConfigurator() throws ConfiguratorException {
		Properties propUser = Configurator.loadPropertiesFromFile(Configurator.PATH_PROPERTIES_USER);
		Properties propSystem = Configurator.loadPropertiesFromResource(Configurator.PATH_PROPERTIES_SYSTEM);
		Properties propNamespace = Configurator.loadPropertiesFromResource(Configurator.PATH_PROPERTIES_NAMESPACE);
		Properties propVersion = Configurator.loadPropertiesFromResource(Configurator.PATH_PROPERTIES_VERSION);
		Properties propAnmerkungen = Configurator.loadPropertiesFromFile(Configurator.PATH_PROPERTIES_ANMERKUNGEN);
		ResourceBundle resStrings = Configurator.loadResourceBundle(Configurator.NAME_RESBUNDLE_STRINGS);
		Configurator config = new Configurator(propUser, propSystem, propNamespace, propVersion, propAnmerkungen,
				resStrings, Configurator.PATH_TAILORING_SCHEMA);
		return config;
	}
	 ...
	 *            Klasse mit allen notwendigen Funktionen für die Oberfläche
	 * @param configurator
	...
	private void initialize() {
		setIconImage(Toolkit.getDefaultToolkit().getImage(
				XsdCreator.class.getResource("/resource/de/drv/dsrv/xtt/gui/icons/app.png")));
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			JMenu mnDatei = new JMenu(configurator.getResString("XSDCREATOR_MENU_DATEI"));
			menuBar.add(mnDatei);

			JMenuItem mntmBeenden = new JMenuItem(configurator.getResString("XSDCREATOR_MENU_BEENDEN"));
			mntmBeenden.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					int n = JOptionPane.YES_OPTION;
					if ((treeMain != null) && (treeMain.getModel() != null)) {
						n = JOptionPane.showConfirmDialog(jContentPane, configurator
								.getResString("XSDCREATOR_DLG_ANPASSUNGEN_VERWERFEN_BEENDEN"), configurator
								.getResString("XSDCREATOR_DLG_BEENDEN"), JOptionPane.YES_NO_OPTION);
					}
					if (n == JOptionPane.YES_OPTION) {
						System.exit(0);
					}
			...
			JMenu mnHilfe = new JMenu(configurator.getResString("XSDCREATOR_MENU_HILFE"));
			menuBar.add(mnHilfe);

			JMenuItem mntmHilfe = new JMenuItem(configurator.getResString("XSDCREATOR_MENU_HILFE"));
			mntmHilfe.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (helpDialog == null) {
						helpDialog = new HelpDialog(XsdCreator.this, configurator);
			...
			JMenuItem mntmber = new JMenuItem(configurator.getResString("XSDCREATOR_MENU_UEBER"));
	...
	private JLabel getLblBezVerfahren() {
		if (lblBezVerfahren == null) {
			...
			lblBezVerfahren.setText(configurator.getResString("XSDCREATOR_LABEL_BEZEICHNUNGVERFAHREN"));
	...
	private JPanel getPanelTree() {
		if (panelTree == null) {
			...
			gbl_panelTree.columnWeights = new double[] { 1.0, 1.0, Double.MIN_VALUE };
	...
	private JTree getTreeMain(ProfilingTreeModel treeModel) {
		...
		try {
			expand = Boolean.parseBoolean(configurator
	...
	private JTree getTreeAdditional(ProfilingTreeModel treeModel) {
		if (treeAdditional == null) {
			...
			treeAdditional.getSelectionModel().setSelectionMode(TreeSelectionModel.SINGLE_TREE_SELECTION);
			treeAdditional.setModel(treeModel);
			treeAdditional.setCellRenderer(new CheckBoxNodeRenderer());
			treeAdditional.setCellEditor(new CheckBoxNodeEditor(treeAdditional));
			...
			btnOkAnmerkungenVerwendung.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if ((currNode != null) && (currNode.getParent() != null)
							&& !currNode.getParent().isRootReferencedElements(configurator)) {
						configurator.setAnmerkungVerwendung(currNode.getSchemaElement(), currNode.getParent()
								.getSchemaElement(), fAnmerkVerwendung.getText());
						fAnmerkVerwendung.setCaretPosition(0);
					}
				}
			});
			btnOkAnmerkungenVerwendung.setIcon(new ImageIcon(XsdCreator.class
	...
	private JPanel getPanel_3() {
		if (panel_3 == null) {
			...
			gbl_panel_3.rowWeights = new double[] { 0.0, 0.0, 0.0, 0.0, 0.5, 0.5 };
			...
			lblSelektiertesElement.setText(configurator.getResString("XSDCREATOR_LABEL_ELEMENT_SELEKTIERT"));
			...
			lblEigenschaften.setText(configurator.getResString("XSDCREATOR_LABEL_ELEMENT_EIGENSCHAFTEN"));
			...
			panel_3.add(jLabelElementEigenschaften, gbc_jLabelElementEigenschaften);
			...
			lblMinOccurs.setText(configurator.getResString("XSDCREATOR_LABEL_MINOCCURS"));
			...
			lblMaxOccurs.setText(configurator.getResString("XSDCREATOR_LABEL_MAXOCCURS"));
			...
			btnMax.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if ((currNode != null) && spinnerMaxOccurs.isEnabled()) {
						spinnerMaxOccurs.setValue(currNode.getMaxOccursDefault());
					}
				}
			});
			...
			lblAnmerkAllg.setText(configurator.getResString("XSDCREATOR_LABEL_ANMERKUNGENALLGEMEIN"));
			...
			btnOkAnmerkungenAllg.setIcon(new ImageIcon(XsdCreator.class
					.getResource("/resource/de/drv/dsrv/xtt/gui/icons/accept.png")));
			btnOkAnmerkungenAllg.setEnabled(false);
			btnOkAnmerkungenAllg.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (currNode != null) {
						configurator.setAnmerkungAllgemein(currNode.getSchemaElement(), fAnmerkAllgemein.getText());
						fAnmerkAllgemein.setCaretPosition(0);
					}
				}
			});
			...
			lblAnmerkVerwendung.setText(configurator.getResString("XSDCREATOR_LABEL_ANMERKUNGENVERWENDUNG"));
			...
			panel_3.add(getBtnOkAnmerkungenVerwendung(), gbc_btnOkAnmerkungenVerwendung);
		}
		return panel_3;
	}
	...
	private JSpinner getSpinnerMinOccurs() {
		if (spinnerMinOccurs == null) {
			...
			spinnerMinOccurs.addChangeListener(new ChangeListener() {
				public void stateChanged(ChangeEvent e) {
					if (spinnerMinOccurs.isEnabled()) {
						// Wert übernehmen
						int currMinValue = Integer.parseInt(spinnerMinOccurs.getValue().toString());
						int currMaxValue = Integer.parseInt(spinnerMaxOccurs.getValue().toString());
						currNode.setMinOccurUser(currMinValue);
						// Minimum f. maxOccurs aktualieren
						// aktuellen Wert setzen
						spinnerMaxOccurs.setModel(new SpinnerNumberModel(currMaxValue, currMinValue, currNode
			...
			spinnerMinOccurs.setModel(new SpinnerNumberModel(new Integer(0), new Integer(0), null, new Integer(1)));
			spinnerMinOccurs.setEnabled(false);
		}
	...
	private void saveKonfig(boolean saveAs) {
		try {
			if (xsdCreatorCtrl.validateBezeichnungKurzVerfahren(fBezKurzVerfahren.getText())) {
				// XML-Konfiguration erstellen
				xsdCreatorCtrl.createXmlProf((ProfilingTreeModel) treeMain.getModel(), getProfilingTreeModelRef(),
						xsdCreatorCtrl.getTargetNamespace(), fBezKurzVerfahren.getText(), fBezVerfahren.getText());
				if (saveAs) {
					JFileChooser fc = new JFileChooser();
					javax.swing.filechooser.FileFilter xmlFileFilter = new XmlXsdFileFilter(XmlXsdFileFilter.SUFFIX.XML);
					fc.setFileFilter(xmlFileFilter);
					fc.setAcceptAllFileFilterUsed(false);
					fc.setSelectedFile(new File(xsdCreatorCtrl.getBezeichnungKurzVerfahren() + ".xml"));
					int returnVal = fc.showSaveDialog(null);
					if (returnVal == JFileChooser.APPROVE_OPTION) {
						File currFile = fc.getSelectedFile();
						int n = JOptionPane.YES_OPTION;
						if (currFile.exists()) {
							...
							n = JOptionPane.showConfirmDialog(jContentPane, configurator
									.getResString("XSDCREATOR_DLG_DATEI_BEREITS_VORHANDEN_FRAGE"), configurator
									.getResString("XSDCREATOR_DLG_DATEI_BEREITS_VORHANDEN"), JOptionPane.YES_NO_OPTION);
						}
						if (n == JOptionPane.YES_OPTION) {
							// XML-Datei speichern
					...
					// XML-Datei speichern
					xsdCreatorCtrl.saveXmlConfig(xsdCreatorCtrl.getPathXmlConfig());
				}
			} else {
				JOptionPane.showMessageDialog(null, configurator
						.getResString("XSDCREATOR_DLG_BEZEICHNUNG_VERFAHREN_FEHLER"), configurator
						.getResString("XSDCREATOR_DLG_SPEICHERN_KONFIG"), JOptionPane.ERROR_MESSAGE);
			}
		} catch (XsdCreatorCtrlException e1) {
			logger.error("Fehler beim Erstellen/Speichern der Profil-Konfiguration:", e1);
			JOptionPane.showMessageDialog(null, configurator
					.getResString("XSDCREATOR_DLG_FEHLER_SPEICHERN_PROFILKONFIGURATION"), configurator
					.getResString("XSDCREATOR_DLG_SPEICHERN_KONFIG"), JOptionPane.ERROR_MESSAGE);
		}
	}
}
